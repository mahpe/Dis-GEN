...
def decode_samples(z_samples,vae_eval,scaler_Y, device='cpu'):
    ...
    save_dict = {'abc':np.round(lattice[:, :3],1),
                 'angles':np.round(lattice[:, 3:6],1),
                 'spacegroup':np.argmax(sg.detach().numpy(),axis=1)+1,
                'disordered_site': F.sigmoid(torch.tensor(disordered_site)).cpu().detach().numpy(),
                'element':element.detach().numpy(),
                'wyckoff_letter':wyckoff_letter.detach().numpy(),
                'wyckoff_mult':wyckoff_multiplier.detach().numpy(),
                'frac_coords':frac_coords.detach().numpy()
                }
    return save_dict
...
def generate_cif_files(all_wyckoffgenes_list,maxiter=None, validity_check=True,charge_neutral=True,
    ...
    for i in tqdm(range(len(all_wyckoffgenes_list))):
        ...
        try:
            ...
            # Unit cell
                ...
                parser = CifParser.from_str(cif_lines, frac_tolerance=5e-2,occupancy_tolerance=2,site_tolerance=5e-2)
                structure = parser.parse_structures(primitive=False)[0]
                structure = Structure.from_sites(structure, properties=structure.properties)
        except:
            failed_count += 1
            continue
    ...
    if maxiter is not None:
        ...
        if symmetry_analyzer:
            print(f'Pymatgen Symmetry failed for {sym_count+failed_count} structures out of {maxiter} Procentage: {100-(sym_count+failed_count)/maxiter*100:.2f}%')
        if validity_check:
            print(f'Validity check failed for {validity_count+failed_count} structures out of {maxiter} Procentage: {100-(validity_count+failed_count)/maxiter*100:.2f}%')
        if charge_neutral:
            print(f'Oxidation state check failed for {oxidation_state_count+failed_count} structures out of {maxiter} Procentage: {100-(oxidation_state_count+failed_count)/maxiter*100:.2f}%')
        print(f'Total valid structures: {len(all_cif_lines)} out of {maxiter} Procentage: {100-len(all_cif_lines)/maxiter*100:.2f}%')
    else:
        ...
        if symmetry_analyzer:
            print(f"Pymatgen Symmetry failed for {sym_count+failed_count} structures out of {len(all_wyckoffgenes_list)} Procentage: {100-(sym_count+failed_count)/len(all_wyckoffgenes_list)*100:.2f}%")
        if validity_check:
            print(f"Validity check failed for {validity_count+failed_count} structures out of {len(all_wyckoffgenes_list)} Procentage: {100-(validity_count+failed_count)/len(all_wyckoffgenes_list)*100:.2f}%")
        if charge_neutral:
            print(f"Oxidation state check failed for {oxidation_state_count+failed_count} structures out of {len(all_wyckoffgenes_list)} Procentage: {100-(oxidation_state_count+failed_count)/len(all_wyckoffgenes_list)*100:.2f}%")
        print(f"Total valid structures: {len(all_cif_lines)} out of {len(all_wyckoffgenes_list)} Procentage: {100-len(all_cif_lines)/len(all_wyckoffgenes_list)*100:.2f}%")
